...
tp_range = np.arange(10, 300.1, 10.0) # $10 to $300 in $10 steps
sessions = [
...
top_exp = res_df.sort_values('expectancy', ascending=False).head(20)

print("--- Top 20 Best Performing Setups (Highest Expectancy) ---")
print(top_exp[['sessions', 'sl', 'tp', 'win_rate', 'expectancy', 'rr', 'net_profit']].to_string(index=False))

# Let's check the absolute best setup for each session group
print("\n--- Best Setup per Session Group ---")
for sess in res_df['sessions'].unique():
    best_sess = res_df[res_df['sessions'] == sess].sort_values('expectancy', ascending=False).head(1)
    print(f"Group: {sess}")
    print(best_sess[['sl', 'tp', 'win_rate', 'expectancy', 'rr', 'net_profit']].to_string(index=False))
    print("-" * 50)
